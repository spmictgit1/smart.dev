...
<div class="card">
    ...
    <div class="card-body">
        ...
        <select id="ddlFruits" onchange="GetSelectedTextValue(this)">
            ...
            <option value="DINI>{{$ps->kod_sekolah}}>{{$ps->ds_nama_sekolah}}">DINI>{{$ps->kod_sekolah}} >
                {{$ps->ds_nama_sekolah}} | QL: {{$ps->quota_L_sabk_dini}} | QP: {{$ps->quota_P_sabk_dini}} |
                IL:{{$ps->JUM_LELAKI}}
                |IP:{{$ps->JUM_PEREMPUAN}} | KL:{{$ps->BEZA_L}} | KP:{{$ps->BEZA_P}} |</option>
            @endforeach
            ...
            <option value="TAHFIZ>{{$ps->kod_sekolah}}>{{$ps->ds_nama_sekolah}}">TAHFIZ>{{$ps->kod_sekolah}} >
                {{$ps->ds_nama_sekolah}} | QL: {{$ps->quota_L_sabk_tahfiz}} | QP: {{$ps->quota_P_sabk_tahfiz}} |
                IL:{{$ps->JUM_LELAKI}}
                |IP:{{$ps->JUM_PEREMPUAN}} | KL:{{$ps->BEZA_L}} | KP:{{$ps->BEZA_P}} |</option>
            @endforeach
                ...
                <table class="table table-bordered table-striped table-hover datatable datatable-Permission">
                    ...
                    <tbody>
                        ...
                        <tr data-entry-id="{{ $datamurid->id }}">
                            ...
                            <td>
                                <p>
                                    <div class="badge badge text-wrap" style="width: 2rem;">{{ $datamurid->id ?? '' }}
                                </p>
                            </td>
                            </td>
                            <td>
                                <div class="badge badge- text-wrap" style="width: ;"> {{$datamurid->PENEMPATAN ??''}}
                                </div>
                            </td>
                            <td>
                                <div class="badge badge- text-wrap" style="width:;">
                                    {{$datamurid->ALIRAN_PENEMPATAN ??''}}</div>
                            </td>
                            <td>
                                <div class="badge badge- text-wrap" style="width: ;" alignment="left">
                                    {{ $datamurid->NAMA ?? '' }}</div>
                            </td>
                            <td>
                                <div class="badge badge-primary text-wrap" style="width: 12rem;"> (KRK):
                                    {{ $datamurid->KRK_NAMA_SEK_DIPOHON?? '' }}</div><br>
                                <div class="badge badge-warning text-wrap" style="width: 12rem;">(KAA):
                                    {{ $datamurid->KAA_NAMA_SEK_DIPOHON?? '' }}</div><br>
                                <div class="badge badge-success text-wrap" style="width: 12rem;">(SABK):
                                    {{ $datamurid->SABK_NAMA_SEK_DIPOHON?? '' }}</div><br>
                            </td>
                            <td>
                                <div class="badge badge-primary text-wrap" style="width: 12rem;"> (P1):
                                    {{ $datamurid->PILIHAN_1?? '' }}</div><br>
                                <div class="badge badge-warning text-wrap" style="width: 12rem;">(P2):
                                    {{ $datamurid->PILIHAN_2?? '' }}</div><br>
                                <div class="badge badge-success text-wrap" style="width: 12rem;"> (P3):
                                    {{ $datamurid->PILIHAN_3?? '' }}</div>
                            </td>

                            <td>
                                <div class="badge badge text-wrap" style="width: ;"> {{ $datamurid->UPSR?? '' }}</div>
                                <br>
                            ...
                            <td>
                                <div class="badge badge-primary text-wrap" style="width:;">
                                    {{ $datamurid->NAMA_SEKOLAH_MEN_LULUS?? '' }}</div>
                            </td>
                            <!--td>
    ...
    <script>
        ...
        $(function () {
            ...
            let deleteButtonTrans = 'TEMPATKAN MURID'
            let deleteButton = {
                ...
                action: function (e, dt, node, config) {
                    ...
                    */
                    if (ids.length === 0) {
                        alert('MAAF.SILA PILIH NAMA MURID DAN NAMA SEKOLAH PENEMPATAN')
                        return
                    }
