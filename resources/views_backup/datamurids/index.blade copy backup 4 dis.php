       ...
       <h4> SILA PILIH SEKOLAH PENEMPATAN:</h4>
        <select id="ddlFruits" onchange="GetSelectedTextValue(this)">
            ...
            <option value="KAA>{{$ps->kod_sekolah}}>{{$ps->ds_nama_sekolah}}">KAA>{{$ps->kod_sekolah}} >
                {{$ps->ds_nama_sekolah}} | Quota L: {{$ps->quota_L_kaa}} | Quota P: {{$ps->quota_P_kaa}} | Isi L:{{$ps->JUM_LELAKI}}
                |Isi P:{{$ps->JUM_PEREMPUAN}} | Kosong L:{{$ps->BEZA_L}} | Kosong P:{{$ps->BEZA_P}} |</option>
            @endforeach

            @foreach ( $listsekolahpilihan_sabk_dini as $ps )
            <option value="DINI>{{$ps->kod_sekolah}}>{{$ps->ds_nama_sekolah}}">DINI>{{$ps->kod_sekolah}} >
                {{$ps->ds_nama_sekolah}} | Quota L: {{$ps->quota_L_sabk_dini}} | QuotaP: {{$ps->quota_P_sabk_dini}} | Isi L:{{$ps->JUM_LELAKI}}
                |Isi P:{{$ps->JUM_PEREMPUAN}} | Kosong L:{{$ps->BEZA_L}} | Kosong P:{{$ps->BEZA_P}} |</option>
            @endforeach

            @foreach ( $listsekolahpilihan_sabk_tahfiz as $ps )
            <option value="TAHFIZ>{{$ps->kod_sekolah}}>{{$ps->ds_nama_sekolah}}">TAHFIZ>{{$ps->kod_sekolah}} >
                {{$ps->ds_nama_sekolah}} | Quota L: {{$ps->quota_L_sabk_tahfiz}} | Quota P: {{$ps->quota_P_sabk_tahfiz}} | Isi L:{{$ps->JUM_LELAKI}}
                |Isi P:{{$ps->JUM_PEREMPUAN}} | Kosong L:{{$ps->BEZA_L}} | Kosong P:{{$ps->BEZA_P}} |</option>
            @endforeach
                     ...
                     {{--   @foreach (array_merge($datamurids,$pbd) as $key =>$datamurid) --}}
                        ...
                        <tr data-entry-id="{{ $datamurid->id }}">
                            ...
                            <td>
                                ...
                                <div class="badge badge-success text-wrap" style="width: 10rem;">{{$datamurid->PENEMPATAN ??''}}</div>
                            </td>
                            <td>
                                <div class="badge badge- text-wrap" style="width: 6rem;">  {{$datamurid->ALIRAN_PENEMPATAN ??''}}</div>
                            </td>
                            <td>
                                <div class="badge badge- text-wrap" style="width: 12rem;">  {{ $datamurid->NAMA ?? '' }}<br>{{ $datamurid->NOKP ?? '' }}</div>
                            </td>
                            <td> 
                            
                                <div class="badge badge-warning text-wrap" style="width: 10rem;">(KAA): {{ $datamurid->KAA_NAMA_SEK_DIPOHON?? '' }}</div><br> 
                                <div class="badge badge-success text-wrap" style="width: 10rem;">(SABK): {{ $datamurid->SABK_NAMA_SEK_DIPOHON?? '' }}</div> 
                            </td>
                            <td>
                                <div class="badge badge-primary text-wrap" style="width: 10rem;"> (P1): {{ $datamurid->PILIHAN_1?? '' }}</div><br>
                                <div class="badge badge-warning text-wrap" style="width: 10rem;">(P2): {{ $datamurid->PILIHAN_2?? '' }}</div><br>
                                 <div class="badge badge-success text-wrap" style="width: 10rem;"> (P3): {{ $datamurid->PILIHAN_3?? '' }}</div>
                            </td>
                            <td>
