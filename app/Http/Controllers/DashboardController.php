...
class DashboardController extends Controller
{
    ...
    {   
      $kira_dashboard_ppd_baru = DB::table('datamurids')
      ->Join('datasekolahs','datasekolahs.kod_sekolah','=','datamurids.KOD_PENEMPATAN')
      ->select (DB::raw('count(datasekolahs.kod_ppd) as PPD_BERJAYA'))
      ->where('datasekolahs.kod_ppd', '=', Auth::user()->kod_organisasi) 
      //->groupby('datamurids.id')
      ->get();

      /*
      ->select('datamurids.KOD_PENEMPATAN','datamurids.id','datasekolahs.kod_ppd','datasekolahs.ppd')
      ->where('datasekolahs.kod_ppd', '=', Auth::user()->kod_organisasi) 
      ->get();*/
  
   //   dd($kira_dashboard_ppd_baru) ;    
       ...
       ->select (DB::raw('
        ...
        ->select (DB::raw('
         ...
         sum(CASE WHEN sahterima = "" THEN 1 ELSE 0 END) as TOLAK
         '))
  ...
  //  dd($kira_dashboard_ppd);
        ...
        return view('/home', compact('kira_dashboard','kira_dashboard_sekolah','kira_terima_tolak','kira_dashboard_ppd_baru'));
    }
    

    public function dashboard_ppd_baru()
    
    {
     $kira_dashboard_baru = DB::table('datamurids')
    ->Join('datasekolahs','datasekolahs.kod_sekolah','=','datamurids.KOD_PENEMPATAN')
    ->select('datamurids.KOD_PENEMPATAN','datamurids.id','datasekolahs.kod_ppd','datasekolahs.ppd')
    ->get();

    dd($kira_dashboard_baru);
    }
